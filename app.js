...
app.get("/user/:idUser", requestHandlers.infoUser); //Esta rota trata as solicitações de login. validaLogin é a função que valida as credenciais
app.post("/user", requestHandlers.registarUser); //solicitações de registro de usuários.
app.put("/user/:idUser", requestHandlers.editarUser); // deletar a conta do site
app.delete("/user/:idUser", requestHandlers.removerUser); // deletar a conta do site

app.get("/user/:id/livro", requestHandlers.getLivrosUser); //busca os livros associados ao utilizador logado
